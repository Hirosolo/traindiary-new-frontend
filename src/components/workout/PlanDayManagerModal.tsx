        ...
        <div className="flex-1 overflow-y-auto p-5 grid lg:grid-cols-2 gap-5">
          <section className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button onClick={startCreate} className="text-xs font-black uppercase tracking-widest text-primary">+ New</button>
            </div>
            <div className="space-y-2">
              ...
              {plans.map((plan) => (
                <div key={plan.plan_id} className="border border-white/10 rounded-xl p-3 bg-surface-card">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <p className="font-bold text-sm">{plan.name}</p>
                      <p className="text-[10px] text-text-dim uppercase tracking-wider">{plan.type || "General"} · {plan.exercises?.length || 0} exercises</p>
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => startEdit(plan)} className="text-[10px] uppercase font-black text-primary">Edit</button>
                      <button onClick={() => void removePlan(plan.plan_id)} className="text-[10px] uppercase font-black text-red-400">Delete</button>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <section className="space-y-3">
            <h3 className="text-xs font-black uppercase tracking-widest text-text-dim">{editingPlanId ? "Edit Plan" : "Create Plan"}</h3>
            ...
            <div className="flex justify-end gap-2 pt-2">
              <button onClick={startCreate} className="px-3 py-2 rounded-lg bg-surface-card border border-white/10 text-xs uppercase font-black">Reset</button>
